...
void insert_score(char [][80], char [][30], char *, int, int, object *, int);
...
void killed_by(object *monster, int other)
{
	...
	if (halluc)
	{
		(void) strcat(buf," (while hallucinating) ");
	} else if (blind)
	{
		(void) strcat(buf," (while blind) ");
	} else if (confused)
	{
		(void) strcat(buf," (while confused) ");
	}
...
void put_scores(object *monster, int other)
{
	...
	clear();
	sprintf(buf,"Top %d Rogueists",MAX_SCORES);
	center ( 3, buf );
